...
echo "BLUETENTACLES installer - $VERSION Version"
...
printf "Host name [bluetentacles-gateway]:"
read NEW_HOSTNAME
if [[ $NEW_HOSTNAME == "" ]]; then NEW_HOSTNAME="bluetentacles-gateway"; fi

printf "Descriptive name [bluetentacles-ic880a]:"
read GATEWAY_NAME
if [[ $GATEWAY_NAME == "" ]]; then GATEWAY_NAME="bluetentacles-ic880a"; fi
...
if [ ! -d lora_gateway ]; then
    git clone https://github.com/geppoleppo/lora_gateway
    pushd lora_gateway
else
    pushd lora_gateway
    git pull
...
if [ ! -d ic880a_packet_forwarder ]; then
    git clone https://github.com/geppoleppo/ic880a_packet_forwarder
...
ln -s $INSTALL_DIR/ic880a_packet_forwarder/poly_pkt_fwd/poly_pkt_fwd ./bin/bluetentacles_pkt_fwd
...
echo -e "{\n\t\"gateway_conf\": {\n\t\t\"gateway_ID\": \"$GATEWAY_EUI\",\n\t\t\"servers\": [ { \"server_address\": \"52.15.232.73:8080\", \"serv_port_up\": 1700, \"serv_port_down\": 1700, \"serv_enabled\": true } ],\n\t\t\"ref_latitude\": $GATEWAY_LAT,\n\t\t\"ref_longitude\": $GATEWAY_LON,\n\t\t\"ref_altitude\": $GATEWAY_ALT,\n\t\t\"contact_email\": \"$GATEWAY_EMAIL\",\n\t\t\"description\": \"$GATEWAY_NAME\" \n\t}\n}" >$LOCAL_CONFIG_FILE
...
cp ./bluetentacles-gateway.service /lib/systemd/system/
cp ./global_conf.json $INSTALL_DIR/bin/global_conf.json
systemctl enable bluetentacles-gateway.service
